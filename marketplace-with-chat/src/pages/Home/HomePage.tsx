...
class HomePage extends React.Component<DefaultProps, DefaultState, object> {
  ...
  public render() {
    const topProductCards = this.props.topProducts.map((product) => {
      return <ProductCard key={product.id} product={product} onClick={this.handleTopProductClick} size={ProductCardSize.Small} hoverable={true} softEdges={false} />;
    });
    const topVendorCards = this.props.topVendors.map((vendor) => {
      return <UserCard key={vendor.id} user={vendor} onClick={this.handleTopVendorClick} size={UserCardSize.Small} color={Color.Transparent} borderless={true} hoverable={true} displayName={true} changeBackgroundColorOnHover={true} />;
    });
    ...
    return (
      <div className="Home">
              ...
              {this.state.shouldDisplayAlert && (
                <Alert bsStyle="warning" onDismiss={this.handleAlertDismiss}>
                  <p>
                    ...
                    <a className="bold-warning" href="https://talkjs.com/tutorials/article/add-buyer-seller-chat-into-a-marketplace-with-react/">
                      tutorial
                    </a>{' '}
                    on how to implement{' '}
                    <a className="bold-warning" href="http://www.talkjs.com">
                      TalkJS
                    </a>{' '}
                    into any React application.
                  </p>
                  <p>Try chatting with Mary!</p>
                </Alert>
              )}
            </div>
            <div className="col-sm-4"></div>
        ...
        <div id="top-vendors-container" className="container">
          <div className="row">
            <div className="col-sm-4"></div>
            <div className="col-sm-4 container-title" id="top-vendor-title">
              Top Vendors
            </div>
            <div className="col-sm-4"></div>
          </div>
