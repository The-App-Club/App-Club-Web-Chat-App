    ...
    const userCards = this.props.users.map((user) => {
      return (
        <div className="col-sm-3" key={user.id}>
          <UserCard user={user} onClick={this.handleUserClick} color={Color.Gray} borderless={false} hoverable={true} size={Size.Small} displayName={true} changeBackgroundColorOnHover={true} />
        </div>
      );
    });
